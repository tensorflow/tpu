...
class ParamsDict(object):
  ...
  def override(self, override_params, is_strict=True, override_with_none=False):
    ...
    Args:
      ...
      override_with_none: a boolean specifying an input value is None for a key,
        whether to override existing values in ParamsDict.
    """
    if self._locked:
    ...
    self._override(override_params, is_strict, override_with_none)  # pylint: disable=protected-access

  def _override(self, override_dict, is_strict=True, override_with_none=False):
    ...
    for k, v in six.iteritems(override_dict):
      ...
      if k not in self.__dict__.keys():
        ...
        elif v is not None or override_with_none:
          self.__dict__[k] = copy.deepcopy(v)
