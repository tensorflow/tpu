...
flags.DEFINE_integer('num_eval_images',
                     default=None,
                     help='Size of evaluation data set.')

flags.DEFINE_integer('num_label_classes',
                     default=None,
                     help='Number of classes, at least 2')
...
flags.DEFINE_bool('transpose_input',
                  default=None,
                  help='Use TPU double transpose optimization')

flags.DEFINE_bool('use_cache',
                  default=None,
...
flags.DEFINE_string(
    ...
    None,
    help=(
        'A string containing comma separated integers indicating ResNet '
...
flags.DEFINE_boolean('pre_activation',
                     default=None,
                     help=('Whether to use pre-activation ResNet (ResNet-v2)'))

flags.DEFINE_string('norm_act_layer',
                    default=None,
...
flags.DEFINE_float('base_learning_rate',
                   default=None,
...
flags.DEFINE_float('poly_rate',
                   default=None,
                   help=('Set LARS/Poly learning rate.'))

flags.DEFINE_bool('use_async_checkpointing',
                  default=None,
                  help=('Enable async checkpoint'))
...
flags.DEFINE_integer('randaug_magnitude',
                     default=None,
...
flags.DEFINE_bool('export_moving_average', False,
                  'Whether to export model using moving average variables.')

flags.DEFINE_bool('amp', False, 'Whether to use automated mixed precision.')

flags.DEFINE_bool('xla', False, 'Whether to use accelerated linear algebra.')
...
flags.DEFINE_integer(
    'infer_steps',
    default=10000,
    help=('Number of batches to plug through the network for inference'))

flags.DEFINE_integer('infer_batch_size',
                     default=32,
                     help=('Batch size for infeernce'))
...
def learning_rate_schedule(params, current_epoch):
  ...
  """
    scaled_lr = params['base_learning_rate'] * (params['train_batch_size'] /
                                                256.0)

    lr_schedule = get_lr_schedule(train_steps=params['train_steps'],
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        if FLAGS.amp:
            optimizer = tf.train.experimental.enable_mixed_precision_graph_rewrite(
                optimizer,
                loss_scale='dynamic'
                if FLAGS.loss_scale == -1 else FLAGS.loss_scale)
...
def _verify_non_empty_string(value, field_name):
  ...
  """
    if not isinstance(value, str):
        raise ValueError('Bigtable parameter "%s" must be a string.' %
                         field_name)
    if not value:
        raise ValueError('Bigtable parameter "%s" must be non-empty.' %
                         field_name)
    return value
...
def main(unused_argv):
    ...
    resnet_classifier = tf.estimator.tpu.TPUEstimator(
        ...
        predict_batch_size=FLAGS.infer_batch_size,
    ...
    if FLAGS.mode == 'eval':
        ...
        while counter < total_steps + warmup_inf_steps:
            for batch in resnet_classifier.predict(
        ...
        print('Throughput: ' + str(FLAGS.infer_batch_size * FLAGS.infer_steps /
