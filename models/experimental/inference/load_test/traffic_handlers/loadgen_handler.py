...
class LoadGenHandler(traffic_handler.TrafficHandler):
  ...
  def start(self):
    ...
    log_settings.log_output.outdir = self._log_dir
  ...
  def process_metrics(self) -> None:
    ...
    messages = self.parse_mlperf_detailed_log()
    self._metrics = {
        "latency": {
            "p50": messages["result_50.00_percentile_latency_ns"]["value"],
            "p90": messages["result_90.00_percentile_latency_ns"]["value"],
            "p95": messages["result_95.00_percentile_latency_ns"]["value"],
            "p99": messages["result_99.00_percentile_latency_ns"]["value"],
        },
    ...
    if "result_completed_samples_per_sec" in messages:
      self._metrics["actual_qps"] = messages[
          "result_completed_samples_per_sec"
      ]["value"]
      logging.info("Actual QPS: %.2f\n", self._metrics["actual_qps"])

    logging.info(
        "latencies in ms: [p50:%.2f p90:%.2f p95:%.2f p99:%.2f]\n",
        self._metrics["latency"]["p50"] / 1e6,
        self._metrics["latency"]["p90"] / 1e6,
        self._metrics["latency"]["p95"] / 1e6,
        self._metrics["latency"]["p99"] / 1e6,
    )

    logging.info(
        "Completed Queries: %d, Failed Queries: %d\n",
        self._completed_queries,
        self._failed_queries,
    )

  def parse_mlperf_detailed_log(self) -> Mapping[str, Any]:
    """Parse the MLPerf detail logs.

    When MLPerf is executed (such as via self.start())
    several log files are dropped in the specified log_dir.  The details log
    includes details about the test, notably including latency percentiles.
    Link to detail log generation:
    mlperf/inference/loadgen/results.cc

    Returns:
      A mapping of the message key to the full message.
    """
    log_path = self._log_dir + "/mlperf_log_detail.txt"
    logging.info("Parsing MLPerf detail log file %s", log_path)
    ...
    logging.info("Successfully loaded MLPerf detail log from %s", log_path)
