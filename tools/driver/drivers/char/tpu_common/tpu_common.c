...
int tpu_common_get_mappable_regions(
 ...
 int (*bar_region_count_cb)(int bar, enum tpu_common_security_level group),
 const struct gasket_mappable_region *(*get_bar_regions_cb)(
  int bar, enum tpu_common_security_level group),
 ...
 *num_mappable_regions =
  bar_region_count_cb(bar_index, target_security_level);
 ...
 *mappable_regions = kzalloc(sizeof(struct gasket_mappable_region) *
 ...
 memcpy(*mappable_regions,
        get_bar_regions_cb(bar_index, target_security_level),
        region_size);
 return 0;
}
