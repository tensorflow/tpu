...
static int gasket_register_interrupt(struct gasket_filp_data *filp_data,
                                     ulong arg);
static int gasket_unregister_interrupt(struct gasket_filp_data *filp_data,
                                       int interrupt);
long gasket_handle_ioctl(struct file *filp, uint cmd, ulong arg)
{
  ...
  if (gasket_dev->driver_desc->ioctl_permissions_cb) {
    retval = gasket_dev->driver_desc->ioctl_permissions_cb(filp, cmd);
    if (retval < 0) {
 ...
 case GASKET_IOCTL_CLEAR_EVENTFD:
  trace_gasket_ioctl_integer_data(arg);
  retval = legacy_gasket_interrupt_clear_eventfd(filp_data, (int)arg);
  break;
...
static int gasket_register_interrupt(struct gasket_filp_data *filp_data,
  ...
  trace_gasket_ioctl_register_interrupt_data(
      mapping.interrupt, mapping.event_fd, mapping.bar_index,
      mapping.reg_offset);
  mutex_lock(&gasket_dev->mutex);
  ret = gasket_interrupt_register_mapping(filp_data, mapping.interrupt,
                                          mapping.event_fd, mapping.bar_index,
                                          mapping.reg_offset);
...
static int gasket_set_event_fd(struct gasket_filp_data *filp_data, ulong arg) {
  ...
  return legacy_gasket_interrupt_set_eventfd(filp_data, die.interrupt,
                                             die.event_fd);
}
...
static int gasket_map_buffers(struct gasket_filp_data *filp_data, ulong arg) {
  ...
  trace_gasket_ioctl_page_table_data(ibuf.page_table_index, ibuf.size,
                                     ibuf.host_address, ibuf.device_address);
  if (ibuf.page_table_index >= gasket_dev->num_page_tables) return -EFAULT;
...
static int gasket_unmap_buffers(struct gasket_filp_data *filp_data, ulong arg) {
  ...
  return gasket_page_table_unmap(gasket_dev->page_table[ibuf.page_table_index],
